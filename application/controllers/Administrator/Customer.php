...
class Customer extends CI_Controller
{
    ...
    public function getCustomers(){
        ...
        $customers = $this->db->query("
            select
                c.*,
                d.District_Name,
                concat(c.Customer_Code, ' - ', c.Customer_Name, ' - ', c.Customer_Mobile, ' - ', c.owner_name) as display_name
            from tbl_customer c
    ...
    public function getCustomerDue(){
        ...
        $dueResult = $this->db->query("
            ...
            from tbl_customer c
            where c.Customer_Type != 'G'  and  c.Customer_brunchid = '$branchId' $customerIdClause
        ")->result();

        echo json_encode($dueResult);
    }

    public function getCustomerDueSms() {

        $dueResult = $this->mt->customerDue();
        $smsData = array_map(function($due) {
            return [
                'to' => $due->Customer_Mobile,
                'message' => "Dear {$due->Customer_Name},\nYour current due is tk. {$due->dueAmount}"
            ];
        }, $dueResult);
