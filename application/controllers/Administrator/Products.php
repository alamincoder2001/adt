...
class Products extends CI_Controller {
        ...
        $stock = $this->db->query("
            select * from(
                select
                    ...
                    ifnull((select sum(purchase_total) as purchase_total 
                            from  tbl_product_serial_numbers 
                            where ps_prod_id = p.Product_SlNo 
                            AND ps_p_r_status <> 'yes' 
                            AND ps_brunch_id = '" . $BRANCHid . "'  
                            AND (ps_s_status IS NULL OR ps_s_status <> 'yes' OR ps_s_r_status = 'yes')
    ...
    public function getSerialByProd(){
          ...
          $BRANCHid = $this->session->userdata("BRANCHid");
          $serials = $this->db->query("SELECT sl.*,prod.Product_Name
