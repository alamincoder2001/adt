...
class Supplier extends CI_Controller {
 ...
 public function getSupplierLedger(){
    ...
    public function supplierPaymentReport($id = Null) {
        $data['title'] = "Supplier Payment Reports";
        ...
        $data['supplier'] = $this->db->query("
            select 
                ...
                s.Supplier_Name,
                s.Supplier_Mobile
            from tbl_supplier_payment as sp
            left join tbl_supplier as s on sp.SPayment_customerID = s.Supplier_SlNo
            where sp.SPayment_status = 'a'
            and sp.SPayment_id = ?
            and sp.SPayment_brunchid = ?
        ", [$id, $this->session->userdata('BRANCHid')])->row();

        $data['content'] = $this->load->view('Administrator/due_report/supplier_payment_report', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
}